...
## 10. 协议与商业模式（2026-08-25 已定）

### 10.1 法律边界

- 已发布的 dsh-continual-evolve v0.1.0–v0.5.0 永远 MIT（npm 与 GitHub 历史不可撤回）；本项目无外部贡献者、唯一版权持有人，未来版本与新产品的协议选择完全自由——新仓启动前正是零成本窗口
- devops-template 提炼自 deepseek-harness 的 11 技能及部分文档为 **MIT 衍生物**：允许并入新协议产品，但必须保留 MIT 版权声明与出处（NOTICE 文件），天然留在免费核心
- 流程卡七张、verify-* 门禁、HANDOFF 纪律、未来引擎内核均为原创，协议自由

### 10.2 分层协议架构（A+D 组合，已采纳）

| 层 | 协议 | 内容 |
|---|---|---|
| `core/` | Apache-2.0 | 方法论骨架 + 七张基础卡 + 安装器；内嵌 MIT 技能文件保留原声明 |
| `pro-content/` | 订阅制商业条款 | 高级垂直卡组、行业模板、进阶治理脚本、抢先体验 |
| `ee/`（Phase 1+） | 商业许可 + license key | 团队治理面板、合规导出、组织策略同步、私有规则注册表 |
| 引擎内核 | Phase 1 时再议是否 AGPL 双许可 | 保留防云白嫖选项 |

核心免费是对抗 superpowers（MIT，★277k）引力的获客武器，不是让利；钱从上层赚。

### 10.3 商业模式：纯产品路线（已采纳）

不做落地咨询服务，专注可规模化收入：

1. **Pro 内容订阅**（主路径，个人开发者低客单）：免费 = 稳定版卡片；订阅 = 高频演化版 + 垂直卡组 + 抢先体验。自进化外环上线后，「订阅最新演化版方法论」成为自然付费理由
2. **ee 团队功能**（远期大头）：对应竞品未占的「Company-level self-evolving」空位——组织级审计导出、合规报告、团队策略分发，license key 解锁
3. 赞助打赏：聊胜于无，不作指望

收款通道：国内个人向爱发电/面包多/知识星球；全球 Gumroad/Lemon Squeezy（商户记录方代缴增值税）；企业向需开发票时注册个体户/公司。定价策略留白至 Phase 0 发布前。

### 10.4 法务卫生清单（新仓第一天执行）

1. `LICENSE`（Apache-2.0）+ `ee/LICENSE` 商业条款（含禁止绕过 license key）
2. `NOTICE` 列明 deepseek-harness MIT 衍生文件清单，技能文件头保留原声明
3. 框架名确定后尽早商标注册（比协议更实际的护城河）
4. 接收外部 PR 之前先公布 CLA/DCO 政策
5. ee/core 边界第一天画死（langfuse 先例：事后拆分极痛）

### 10.5 开放捐助渠道（2026-08-25 补充）

定位：信号塔与 README 可信度组件，非现金流主力（主路径见 §10.3）。

| 渠道 | 市场 | 要点 |
|---|---|---|
| 爱发电 | 国内主力 | 费率 6%（1% 支付通道 + 5% 平台），微信/支付宝；月度档+一次性；中文开源事实标准 |
| 微信/支付宝赞赏码 | 国内 | README 折叠内嵌，零费率惯例 |
| 知识星球 | 国内可选 | 后期付费社区形态，先不开 |
| GitHub Sponsors | 国际 | 大陆个人暂不可用（社区通行做法是引导至爱发电）；待有海外收款主体（港/美实体）再开 |
| Ko-fi / Open Collective | 国际备选 | 均依赖 PayPal/Stripe 或 fiscal host，大陆个人受限——开户前逐个核实现行政策 |
| Lemon Squeezy / Gumroad | 全球 | Pro 销售通道顺带承接「用购买表达支持」，不单开捐助 |

落地件：`.github/FUNDING.yml` 用 `custom` 字段挂爱发电链接；双语 README「赞赏/Support」节；资金用途声明（开发时间/服务器/模型调用成本）。合规：小额个人受赠无感，量级起来后随主体化一并处理结汇与税务。
